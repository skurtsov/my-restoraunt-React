  ...
  function handleDecrement() {
    ...
    return(
        ...
        <h3 className='hidden' ref={nameRef}>{props.state.name}</h3>
            <h3 ref={nameLoc}>{props.state.name_fr}</h3>
         
        </div>
        <div className="desc">
            <p>{props.state.desc_fr}</p>
        </div>
        <div className="price">
            <p ref={priceRef}>{props.state.price} Euro</p>
        </div>
        
       {!hiddenZak ? <div className="subm-button" onClick={goZakaz}>Le Order</div>:null}
        <br />
